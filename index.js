...
async function Data(address, size) {
  let payload = {
  ...
  const {data} = await axios.post(`https://www.vrbo.com/serp/g`, payload, {
    headers: {
    ...
    var csv="ListingId,\tHeadLine,\t"

    let today = new Date();
    let endArrDates= [];
    listings.forEach(obj => {
      endArrDates.push(new Date(obj.rateSummary.endDate))
    });
    const maxEndDate = new Date(Math.max(...endArrDates));
    
      let t = maxEndDate.getTime() + (1000 * 3600 * 24)
      for(let d = new Date(); d <= t; d.setDate(d.getDate() + 1)){
        let tempDate = new Date(d)
        if(d.toLocaleDateString() === maxEndDate.toLocaleDateString()) {
          csv += `${tempDate.getDate()}-${tempDate.getMonth()}-${tempDate.getFullYear()} \t`

        }
        else {csv += `${tempDate.getDate()}-${tempDate.getMonth()}-${tempDate.getFullYear()}, \t`}
      }
      csv += '\n'
    listings.forEach((obj)=>{

      csv += `${obj.listingId},\t`
      let x = obj.propertyMetadata.headline.split(',').join(' ')
      csv += `${x},\t`

      //calculate total time difference from today to End date
      const diffTime = Math.abs(today.getTime() - maxEndDate.getTime());
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      
      if(!obj.rateSummary.rentNights){
        ...
        let len= obj.rateSummary.rentNights.length;
        let startDate= new Date(obj.rateSummary.beginDate)
      //calculate time difference
      const diffTime = Math.abs(today - startDate);
      const diffDay = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
         for(let i=diffDay; i<len; i++){
           if(i === len - 1) {
            csv += obj.rateSummary.rentNights[i];
           }
          else csv += obj.rateSummary.rentNights[i]+ ',' +"\t";
         }
        csv = csv+"\n"

      }
      fsLibrary.writeFile('newfile.csv', csv, (error) => {
           if (error) throw err;
       })
 })
  })
...
Data("chicago-illinois-united-states-of-america", 50)